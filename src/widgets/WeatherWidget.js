  ...
  getProperties() {
    ...
    switch (device.dtype) {
      case "Temp" :
        return { temp: temp };
      case "Humidity" :
        return { humonly: Number(device.nvalue) + 1, humstat: device.svalue1 };
      case "Temp + Humidity" :
    ...
    return (
      <div className="WeatherWidget">
        ...
        <div className={'weatherData'} style={style}>
          ...
          {params.humonly && <div className="humonly">{Number(params.humonly) - 1}</div>}
		      {params.humstat && <div className="humstat">{humidityStatus}</div>}
        </div>
      </div>
    );
  }
